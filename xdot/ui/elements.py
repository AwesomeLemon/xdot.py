...
class Graph(Shape):
    ...
    def _draw_nodes(self, cr, bounding, highlight_items):
        new_highlight_items = set()
        if len(highlight_items) > 0:
            moused_over_item = list(highlight_items)[0]
            if type(moused_over_item) == Node:
                new_highlight_items_in = set()
                new_highlight_items_out = set()
                for edge in self.edges:
                    if edge.dst == moused_over_item:
                        new_highlight_items_in.add(edge.src)
                    if edge.src == moused_over_item:
                        new_highlight_items_out.add(edge.dst)
                new_highlight_items = new_highlight_items_in.union(new_highlight_items_out)

                cur_highlight_items_higher_order_in = set()
                prev_highlight_items_higher_order_in = new_highlight_items_in
                for i in range(20):
                    for edge in self.edges:
                        if edge.dst in prev_highlight_items_higher_order_in:
                            cur_highlight_items_higher_order_in.add(edge.src)
                    new_highlight_items = new_highlight_items.union(cur_highlight_items_higher_order_in)
                    if len(cur_highlight_items_higher_order_in) == 0:
                        break

                    prev_highlight_items_higher_order_in = cur_highlight_items_higher_order_in
                    cur_highlight_items_higher_order_in = set()

                cur_highlight_items_higher_order_out = set()
                prev_highlight_items_higher_order_out = new_highlight_items_out
                for i in range(20):
                    for edge in self.edges:
                        if edge.src in prev_highlight_items_higher_order_out:
                            cur_highlight_items_higher_order_out.add(edge.dst)
                    new_highlight_items = new_highlight_items.union(cur_highlight_items_higher_order_out)
                    if len(cur_highlight_items_higher_order_out) == 0:
                        break

                    prev_highlight_items_higher_order_out = cur_highlight_items_higher_order_out
                    cur_highlight_items_higher_order_out = set()

        for node in self.nodes:
            if bounding is None or node._intersects(bounding):
                node._draw(cr, highlight=(node in highlight_items or node in new_highlight_items), bounding=bounding)
